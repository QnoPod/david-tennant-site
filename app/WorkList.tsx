    ...
    <main className={styles.main}>
      <div className={styles.header}>
        ...
        <div className={styles.topNav}>
          <Link href="/characters" className={`${styles.actionBtn} ${styles.actionBtnPrimary}`}>
            <span style={{ color: '#4a38df', fontSize: '16px' }}>👥</span> キャラクターリストを見る
          </Link>
          ...
          <button className={styles.actionBtn} onClick={() => setShowAboutModal(true)}>
             <span style={{ color: '#7aa5d2', fontSize: '16px' }}>ℹ️</span> サイトについて・更新履歴
          </button>
        </div>
              ...
              return (
                <div key={`${work.id}-${index}`} className={styles.timelineItem}>
                  ...
                  <div className={styles.timelineContent} onClick={() => setSelectedWork(work)}>
                    {work.poster_path ? (
                      <img src={`https://image.tmdb.org/t/p/w200${work.poster_path}`} alt={work.title || work.name} style={{ width: '80px', height: '120px', objectFit: 'cover', borderRadius: '4px', flexShrink: 0 }} />
                    ) : (
                      <div style={{ width: '80px', height: '120px', backgroundColor: '#0a0a0c', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: '24px', opacity: 0.5 }}>🎬</div>
                    )}
                    <div>
      ...
      {showAboutModal && (
        <div onClick={() => setShowAboutModal(false)} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ backgroundColor: '#16161a', padding: '40px', borderRadius: '12px', maxWidth: '600px', width: '100%', position: 'relative', border: '1px solid rgba(255,255,255,0.05)', boxShadow: '0 20px 50px rgba(0,0,0,0.8)', maxHeight: '80vh', overflowY: 'auto' }}>
            <button onClick={() => setShowAboutModal(false)} style={{ position: 'absolute', top: '20px', right: '20px', background: 'none', border: 'none', color: '#666', fontSize: '24px', cursor: 'pointer', transition: 'color 0.2s' }} onMouseEnter={e => e.currentTarget.style.color = '#fff'} onMouseLeave={e => e.currentTarget.style.color = '#666'}>✕</button>
            <h2 style={{ color: '#d4af37', marginTop: 0, marginBottom: '20px', borderBottom: '1px solid #333', paddingBottom: '15px', fontWeight: '500' }}>ℹ️ サイトについて</h2>
            <p style={{ color: '#d0d0d0', fontSize: '15px', lineHeight: '1.9', marginBottom: '40px', letterSpacing: '0.03em' }}>
              当サイトは、デヴィッド・テナントの出演作品およびキャラクターの情報をまとめた非公式のファンデータベースです。<br />
              配信状況の確認や、各キャラクターの詳細設定を振り返るのにお役立てください。
            </p>
            <h2 style={{ color: '#d4af37', margin: '0 0 20px 0', borderBottom: '1px solid #333', paddingBottom: '15px', fontWeight: '500' }}>🕒 更新履歴</h2>
            <ul style={{ color: '#d0d0d0', fontSize: '14px', lineHeight: '2.2', paddingLeft: '20px', margin: 0 }}>
      ...
      <ScrollButtons />
      <footer style={{ textAlign: 'center', marginTop: '60px', paddingBottom: '20px', color: '#555', fontSize: '14px', letterSpacing: '0.1em' }}>
