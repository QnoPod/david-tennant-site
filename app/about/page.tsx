...
export default async function AboutPage() {
  const [upcoming, conventions] = await Promise.all([getUpcomingWorks(), getConventionAppearances()]);
  const siteUpdates = buildAutomaticSiteUpdates({ interviews: getInterviewsNewestFirst(), upcoming, conventions });
  return <main id="main-content"><PageHero eyebrow="ABOUT THIS ARCHIVE" title="ABOUT" description="このサイトの情報源、更新内容について。" />
    ...
    <section className="section section--paper"><div className="shell"><div className="section-heading"><div><p className="eyebrow">CHANGELOG</p><h2>更新履歴</h2></div></div><div className="updates-list">{siteUpdates.map((item) => <div key={`${item.date}-${item.text}`}><time>{item.date.replaceAll("-", ".")}</time><p><span>{item.category}</span><Link href={item.href}>{item.text}</Link></p></div>)}</div></div></section>
  </main>;
}
