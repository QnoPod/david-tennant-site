...
export default function CharacterSortPage() {
  const sortState = useCharacterSort();

  if (!sortState.matchUp && !sortState.showRanking) {
    return <div style={{ color: '#eaeaea', textAlign: 'center', marginTop: '50px' }}>Loading...</div>;
  }
    ...
    <main className={styles.container}>
      <style>{`
        ...
        @media (max-width: 600px) {
          ...
          .char-image-wrap { width: 80px; height: 80px; margin: 0 auto 10px auto; }
          ...
          .vote-btn-primary { font-size: 12px !important; padding: 14px 6px !important; margin-bottom: 8px !important; }
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        {sortState.matchUp && !sortState.showRanking ? (
          <MatchUpBoard {...sortState} matchUp={sortState.matchUp} />
        ) : (
          <RankingBoard 
            ranking={sortState.ranking}
            sortTheme={sortState.sortTheme}
            totalVotes={sortState.totalVotes}
            activeCount={sortState.activeCount}
            setShowRanking={sortState.setShowRanking}
            handleReset={sortState.handleReset}
          />
        )}
      </div>
    </main>
