...
import CharacterGridView from '../components/views/CharacterGridView';
import CharacterTimelineView from '../components/views/CharacterTimelineView';
...
export default function CharacterList({ tmdbWorks }: { tmdbWorks: any[] }) {
  ...
  const toggleFavorite = (char: any, e: React.MouseEvent) => {
    ...
    const newFavs = favoriteChars.includes(key) 
      ? favoriteChars.filter(k => k !== key) 
      : [...favoriteChars, key];
