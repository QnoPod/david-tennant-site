...
export default function ArchiveControls({ query, onQueryChange, view, onViewChange, count, children }: { query: string; onQueryChange: (value: string) => void; view: "grid" | "timeline"; onViewChange: (value: "grid" | "timeline") => void; count: number; children?: React.ReactNode }) {
  return (
    <div className="archive-controls">
      ...
      <div className="archive-controls__actions">{children}<button className={`archive-view-button ${view === "grid" ? "is-active" : ""}`} onClick={() => onViewChange("grid")}>グリッド</button><button className={`archive-view-button ${view === "timeline" ? "is-active" : ""}`} onClick={() => onViewChange("timeline")}>年代順</button><span>{count}件</span></div>
    </div>
  );
}
