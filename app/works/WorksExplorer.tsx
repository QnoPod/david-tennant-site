 ...
 * 検索条件はURLにも保存し、再読み込み・共有・戻る／進むで復元します。
 */
export default function WorksExplorer({ works }: { works: Work[] }) {
  ...
  const [sortOrder, setSortOrder] = useState<SortOrder>(() =>
    readEnumParam(
      searchParams,
      "sort",
      ["default", "popularity", "title"] as const,
      "default",
    ));
  ...
  const [selectedProviders, setSelectedProviders] = useState<string[]>(() =>
    readListParam(searchParams, "provider"));
  const [selectedGenres, setSelectedGenres] = useState<string[]>(() =>
    readListParam(searchParams, "genre"));
  ...
  const providers = useMemo(
    () => [...new Set(
      uniqueWorks.flatMap((work) =>
        work.providers?.map((provider) => provider.provider_name) ?? []),
  ...
  const genres = useMemo(
    () => [...new Set(
      uniqueWorks.flatMap((work) =>
        work.genres?.map((genre) => genre.name) ?? []),
    )].sort((a, b) => a.localeCompare(b, "ja")),
    [uniqueWorks],
  );

  // 共有URLやブラウザの戻る／進むで変化したURLを画面へ反映します。
  useEffect(() => {
    const params = new URLSearchParams(searchString);
    const next: WorkUrlState = {
      query: params.get("q") ?? "",
      characterQuery: params.get("character") ?? "",
      ...
      expanded: readBooleanParam(params, "details"),
      selectedProviders: readListParam(params, "provider"),
      selectedGenres: readListParam(params, "genre"),
      genreMode: readEnumParam(
        params,
        "genreMode",
        ["include", "exclude"] as const,
        "include",
    ...
    const changed = next.query !== current.query
      || next.characterQuery !== current.characterQuery
      ...
      || !sameStringList(next.selectedProviders, current.selectedProviders)
      || !sameStringList(next.selectedGenres, current.selectedGenres)
      || next.genreMode !== current.genreMode
  ...
  // 入力やフィルター変更をURLへ保存します。履歴を増やしすぎないようreplaceを使います。
  useEffect(() => {
    if (syncingFromUrl.current) return;

    const params = new URLSearchParams(searchString);
    setStringParam(params, "q", query);
    setStringParam(params, "character", characterQuery);
    setStringParam(params, "availability", availability, "ALL");
    setStringParam(params, "watch", watchStatus, "ALL");
    setStringParam(params, "sort", sortOrder, "default");
    setBooleanParam(params, "favorites", favoritesOnly);
    setBooleanParam(params, "details", expanded);
    setListParam(params, "provider", selectedProviders);
    setListParam(params, "genre", selectedGenres);
    setStringParam(params, "genreMode", genreMode, "include");
  ...
  const filtered = useMemo(() => {
    ...
    const result = uniqueWorks.filter((work) => {
      ...
      const characterText = normalizeText([
        work.character,
        ...getWorkCharacters(work).flatMap((character) => [
          character.name,
    ...
    if (view === "timeline" || sortOrder === "default") {
      return result.sort((a, b) =>
        getWorkDate(b).localeCompare(getWorkDate(a)));
    }
    ...
    return result.sort((a, b) =>
      getDisplayTitle(a).localeCompare(getDisplayTitle(b), "ja"));
  }, [
    availability,
    characterQuery,
    favorites,
  ...
  const toggleFavorite = (work: Work) => {
    ...
    if (
      isFavorite
      && !window.confirm(`「${getDisplayTitle(work)}」のお気に入りを解除しますか？`)
    ) return;
    saveList(
      FAVORITES_KEY,
      isFavorite
        ? favorites.filter((id) => id !== work.id)
        : [...favorites, work.id],
      setFavorites,
    );
  };

  const toggleWatched = (work: Work) => saveList(
    WATCHED_KEY,
    watched.includes(work.id)
      ? watched.filter((id) => id !== work.id)
      : [...watched, work.id],
    setWatched,
  );
  ...
  const toggleWatchLater = (work: Work) => {
    ...
    writeArchiveList(
      WATCH_LATER_KEY,
      watchLater.includes(key)
        ? watchLater.filter((item) => item !== key)
        : [...watchLater, key],
    );
  };
  ...
  const card = (work: Work, timeline = false) => {
    ...
    const hasSpiteloutAndIvar = characters.some((character) =>
      normalizeText(character.englishName).includes("spitelout"))
      && characters.some((character) =>
        normalizeText(character.englishName).includes("ivarthewhitless"));
    const isFavorite = favorites.includes(work.id);
    const isWatched = watched.includes(work.id);
    const isWatchLater = watchLater.includes(
      getWorkArchiveKey(work.media_type, work.id),
    );
    const rating = ratings[getWorkArchiveKey(work.media_type, work.id)];

    return (
      <article
        className={timeline ? "work-timeline-card" : "media-card work-card"}
      ...
      >
        ...
        <div className="work-card__image">
          ...
          />
          <button
            className={`work-status work-status--watched ${isWatched ? "is-active" : ""}`}
            onClick={() => toggleWatched(work)}
            aria-label={isWatched ? "視聴済みを解除" : "視聴済みにする"}
          >
            ✓
          </button>
          <button
            className={`work-status work-status--favorite ${isFavorite ? "is-active" : ""}`}
          ...
          <button
            className={`work-status work-status--later ${isWatchLater ? "is-active" : ""}`}
            onClick={() => toggleWatchLater(work)}
            aria-label={isWatchLater ? "あとで見るから外す" : "あとで見るに追加"}
            title="あとで見る"
          >
            ◷
          </button>
        ...
        <div className="work-card__body">
          ...
          {rating && (
            <span
              ...
              aria-label={`自分の評価 ${rating}点、5点満点`}
            >
              ★ {rating}/5
            </span>
          )}
          ...
          {hasSpiteloutAndIvar
            ? (
              <div className="work-card__character-lines">
                {characters.map((character) => (
                  <span key={character.englishName}>{character.name}</span>
                ))}
              </div>
            )
  ...
  return (
    <section className="archive-section shell">
      <WorkFilters
        ...
        sortOrder={view === "timeline" ? "default" : sortOrder}
        ...
        onClearFavorites={() => {
          if (
            !favorites.length
            || !window.confirm("WORKSのお気に入りをすべて解除しますか？")
          ) return false;
      ...
      <div className="archive-summary">
        <p>カードを選ぶと配信情報・予告編・役柄の詳細を表示します。</p>
        <div>
          <button
          ...
          <strong>{filtered.length} / {uniqueWorks.length}作品</strong>
        </div>
      </div>

      {view === "grid"
        ? <div className="media-grid">{filtered.map((work) => card(work))}</div>
        : (
          <div className="work-timeline">
            {timelineGroups.map(([year, yearWorks]) => (
              <section className="timeline-year-group" key={year}>
                <h2>{year}</h2>
                <div>{yearWorks.map((work) => card(work, true))}</div>
              </section>
            ))}
          </div>
        )}
      {!filtered.length && (
        <p className="empty-state">条件に一致する作品がありません。</p>
      )}

      <WorkDetailModal
        work={selected}
        watched={selected ? watched.includes(selected.id) : false}
        watchLater={selected
          ? watchLater.includes(getWorkArchiveKey(selected.media_type, selected.id))
          : false}
        rating={selected
          ? ratings[getWorkArchiveKey(selected.media_type, selected.id)] ?? 0
          : 0}
        onToggleWatched={() => selected && toggleWatched(selected)}
        onToggleWatchLater={() => selected && toggleWatchLater(selected)}
        onRate={(nextRating) =>
          selected && setWorkRating(
            selected.media_type,
            selected.id,
            nextRating,
          )}
...
function WorkDetailModal({
  ...
  const originalTitleForDisplay = work
    ? getOriginalTitleForDisplay(work)
    : null;
  const relatedInterviews = work
    ? findRelatedInterviews([
        displayTitle,
        getSourceTitle(work),
        originalTitle,
  ...
  const shareHref = work
    ? `/works/share/${getWorkSlug(work)}?card=3`
    : "/works";
    ...
    <Modal open={Boolean(work)} onClose={onClose} label={`${displayTitle}の詳細`}>
      {work && (
        <div className="work-detail">
          {(work.backdrop_path || work.backdropUrl) && (
            <div
              className="work-detail__backdrop"
              style={{
                backgroundImage:
                  `linear-gradient(to top, var(--white), transparent), url('${getBackdropUrl(work.backdrop_path, work.backdropUrl)}')`,
              }}
            />
          )}
          <header>
            ...
            <h2>{displayTitle}</h2>
            {originalTitleForDisplay && (
              <p className="detail-subtitle">{originalTitleForDisplay}</p>
            )}
            <div className="work-detail__actions">
              <button
                className={`detail-watch ${watched ? "is-active" : ""}`}
                onClick={onToggleWatched}
              >
                {watched ? "✓ 視聴済" : "▷ 未視聴"}
              </button>
              <button
                className={`detail-later ${watchLater ? "is-active" : ""}`}
                onClick={onToggleWatchLater}
              >
                {watchLater ? "◷ あとで見るに保存済み" : "◷ あとで見る"}
              </button>
            </div>
          </header>

          <div className="work-detail__facts">
            {work.media_type !== "stage" && (
              <span>
                {work.media_type === "movie"
                  ? (work.runtime ? `${work.runtime}分` : "映画")
                  : `${work.numberOfSeasons
                    ? `全${work.numberOfSeasons}シーズン`
                    : "TV番組"}${work.numberOfEpisodes
                    ? ` · ${work.numberOfEpisodes}話`
                    : ""}${work.episodeRunTime
                    ? ` · 1話約${work.episodeRunTime}分`
                    : ""}`}
              </span>
            )}
          ...
          <section className="detail-section">
            <h3>演じたキャラクター</h3>
            <div className="work-characters">
              {characters.map((character) => (
                <article key={`${character.name}-${character.englishName}`}>
                  <img
                    src={character.image}
                    alt={character.name}
                    width="400"
                  ...
                  <div>
                    ...
                    {character.appearanceNote && (
                      <p className="work-character__appearance">
                        <strong>出演形態：</strong>{character.appearanceNote}
                      </p>
                    )}
                    <p>{character.description}</p>
                    {!character.excludeFromCharacters && (
                      <Link
                        className="text-link"
...
function WorkRatingControl({
  ...
  return (
    <section className="work-rating" aria-labelledby="work-rating-title">
      <div>
        <p className="eyebrow">MY RATING</p>
        <h3 id="work-rating-title">自分の評価</h3>
        <small>このブラウザ内だけに保存されます。</small>
      </div>
      <div
        className="work-rating__stars"
        role="group"
        aria-label={`${title}の5段階評価`}
      >
...
function EpisodeAppearances({ work }: { work: Work }) {
  const manualResult = work.episodeAppearances?.length
    ? {
        ...
        appearances: work.episodeAppearances,
        episodeCount: work.episodeAppearances.length,
      }
    : null;
  const [result, setResult] = useState<EpisodeAppearanceResult | null>(
    manualResult,
  );
  ...
  const getEpisodeProgressKey = (
    episode: EpisodeAppearanceResult["appearances"][number],
  ) => `${progressPrefix}${episode.seasonNumber}:${episode.episodeNumber}:${
    episode.airDate || ""
  }:${normalizeText(
    episode.title || episode.displayLabel || "",
  )}`;

  useEffect(() => {
    const sync = () => setWatchedEpisodeKeys(
      readArchiveList<string>(ARCHIVE_STORAGE_KEYS.watchedEpisodes),
    );
    sync();
    window.addEventListener("storage", sync);
    window.addEventListener(ARCHIVE_UPDATED_EVENT, sync);
  ...
  useEffect(() => {
    ...
    [getSourceTitle(work), work.original_name, work.name]
      .filter((title): title is string => Boolean(title))
      .forEach((title) => params.append("title", title));

    fetch(`/api/tmdb/episode-appearances?${params}`, {
  ...
  const watchedCount = episodeKeys.filter((key) =>
    watchedEpisodeKeys.includes(key)).length;
  ...
  const toggleEpisode = (key: string) => writeArchiveList(
    ...
    watchedEpisodeKeys.includes(key)
      ? watchedEpisodeKeys.filter((item) => item !== key)
      : [...watchedEpisodeKeys, key],
  );

  const toggleAllEpisodes = () => {
    const otherWorks = watchedEpisodeKeys.filter((key) =>
      !key.startsWith(progressPrefix));
    writeArchiveList(
      ARCHIVE_STORAGE_KEYS.watchedEpisodes,
      watchedCount === episodeKeys.length
        ? otherWorks
        : [...otherWorks, ...episodeKeys],
    );
  };

  return (
    <section className="detail-section episode-appearances">
      <div className="episode-appearances__heading">
        <h3>出演エピソード</h3>
        <span>{result.appearances.length}件確認済み</span>
      </div>
      <div className="episode-progress">
        <div>
          <strong>出演回の視聴進捗</strong>
          <span>{watchedCount} / {episodeKeys.length}話</span>
          <button type="button" onClick={toggleAllEpisodes}>
            {watchedCount === episodeKeys.length
              ? "すべて解除"
              : "すべて視聴済み"}
          </button>
        ...
        <div
          className="episode-progress__bar"
          role="progressbar"
          aria-label="出演回の視聴進捗"
          aria-valuemin={0}
          aria-valuemax={episodeKeys.length}
      ...
      <ol>
        {result.appearances.map((episode) => {
          const progressKey = getEpisodeProgressKey(episode);
          const isWatched = watchedEpisodeKeys.includes(progressKey);
          const displayLabel = episode.displayLabel
            || `S${episode.seasonNumber} E${episode.episodeNumber}`;
          return (
            <li
              className={isWatched ? "is-watched" : undefined}
              key={progressKey}
            >
              <strong>{displayLabel}</strong>
              {episode.title && <span>「{episode.title}」</span>}
              {episode.airDate && (
                <time dateTime={episode.airDate}>{episode.airDate}</time>
              )}
              <button
                type="button"
                aria-pressed={isWatched}
                aria-label={`${displayLabel}を${
                  isWatched ? "未視聴に戻す" : "視聴済みにする"
                }`}
                onClick={() => toggleEpisode(progressKey)}
              >
                {isWatched ? "✓ 視聴済み" : "未視聴"}
              </button>
